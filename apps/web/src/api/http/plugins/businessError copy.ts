...
// import type { CustomAxiosError, IHttpPlugin, IPluginError } from '../IHttp'
// import type { IEventBusEvents } from '@/eventBus/IEventBus'
// import { eventBus } from '@/eventBus'
// import { AxiosUtils } from '../HttpUtils'
...
//   if (String((error.response?.data as IErrorResponse).code).length <= 3) return null
//   const {
...
//   const _message = `${message !== '' ? `${message}:` : ''}${(error.response?.data as IErrorResponse)?.msg}`
